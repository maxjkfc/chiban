...
三個固定項目：群組 / 記錄 / 我的。記錄是工具列正中央唯一的主要行動
鍵，使用較大的珊瑚色圓形按鈕；群組與我的以低權重 icon + label 放在兩側。
這讓拍照記錄成為清楚的唯一入口，不再同時呈現記錄與新增兩個語意重疊的
按鈕。

Today 仍是 meal history/today page，但不是競爭主要行動的 bottom-bar
item，從 app root 與 meal links 進入。兩側分頁目前仍以**珊瑚色文字 + 底下
一條短線**標示，不是填滿的膠囊；顏色不是唯一線索：圖示放大、文字加粗、
短線出現，三個一起。
